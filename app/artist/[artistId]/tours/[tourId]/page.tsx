...
export default function ArtistEditTourPage({
  params,
}: {
  params: Promise<{ artistId: string; tourId: string }>;
}) {
  const { artistId, tourId } = use(params);
  ...
  async function fetchData() {
    ...
    try {
      ...
      if (!tourRes.ok) {
        if (tourRes.status === 404 || tourRes.status === 403) {
          router.push(`/artist/${artistId}/tours`);
          return;
        }
  ...
  async function handleAddCountry(countryCode: string) {
    ...
    try {
      const res = await fetch(`/api/tours/${tourId}/countries`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleRemoveCountry() {
    ...
    try {
      const res = await fetch(
        `/api/tours/${tourId}/countries?override_id=${overrideId}`,
        { method: "DELETE" },
      );
  ...
  function getMDERecommendation(
    ...
    const dateSpecific = countryDefaults.find(
      (d) =>
        ...
        (!d.effective_to || d.effective_to >= today),
    );
    if (dateSpecific) return dateSpecific;
    return (
      countryDefaults.find(
        (d) => d.country_code === countryCode && !d.effective_from,
      ) || null
    );
  }
  ...
  const existingCountries = new Set(overrides.map((o) => o.country_code));
  const availableCountries = COUNTRY_OPTIONS.filter(
    (c) => !existingCountries.has(c.value),
  );
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <Popover open={countryOpen} onOpenChange={setCountryOpen}>
                <PopoverTrigger asChild>
                  <Button size="sm" disabled={addingCountry}>
                    Add Country
                  </Button>
                </PopoverTrigger>
                ...
                <Table>
                  ...
                  <TableBody>
                    {overrides.map((override) => {
                      const mdeRecommendation = getMDERecommendation(
                              ...
                              >
                                ...
                                <SelectContent>
                                  <SelectItem value="none">
                                    ...
                                    >
                                      {mdeRecommendation
                                        ? `Use MDE recommended (${mdeRecommendation.org?.org_name})`
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete &ldquo;{tour.name}&rdquo;? This
      ...
      <Dialog
        ...
        onOpenChange={(open) => !open && setRemoveCountryTarget(null)}
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              from this tour? Fans from this country will no longer be routed to
              an organization during this tour.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
